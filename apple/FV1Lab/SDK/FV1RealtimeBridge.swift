...
final class FV1RealtimeBridge: @unchecked Sendable {
    ...
    func configure(inputRate: Double, outputRate: Double) throws {
        ...
        if result != FV1_SDK_OK {
            throw FV1EngineError.sdk(result, "Unable to configure Apple audio rates")
        }
    }

    func load(program: Data) throws {
        guard program.count == Int(FV1_SDK_PROGRAM_BYTES) else {
            throw FV1EngineError.invalidProgramSize(program.count)
        }
        let result = program.withUnsafeBytes { bytes -> fv1_sdk_result in
            guard let base = bytes.bindMemory(to: UInt8.self).baseAddress else {
                return fv1_sdk_result(FV1_SDK_ERROR_INVALID_ARGUMENT)
            }
            return fv1_apple_realtime_load_program(handle, base, bytes.count)
        }
        if result != FV1_SDK_OK {
            throw FV1EngineError.sdk(result, "Unable to load realtime program")
        }
    }
    ...
    func setDSPEnabled(_ enabled: Bool) {
        fv1_apple_realtime_set_dsp_enabled(handle, enabled ? 1 : 0)
    }

    var dspEnabled: Bool {
        fv1_apple_realtime_get_dsp_enabled(handle) != 0
    }
    ...
    func reset() throws {
        ...
        if result != FV1_SDK_OK {
            throw FV1EngineError.sdk(result, "Unable to reset realtime engine")
        }
    }
    ...
    func analysis(
        ...
        var metadata = fv1_apple_analysis_snapshot_v1()
        fv1_apple_analysis_snapshot_v1_init(&metadata)

        var spectrum = [Float](repeating: -200, count: maxSpectrumBins)
        var left = [Float](repeating: 0, count: maxScopeFrames)
        var right = [Float](repeating: 0, count: maxScopeFrames)

        let result = spectrum.withUnsafeMutableBufferPointer { spectrumBuffer in
        ...
        guard result == FV1_SDK_OK else { return .empty }

        let spectrumCount = min(Int(metadata.spectrum_bins), spectrum.count)
        let scopeCount = min(Int(metadata.scope_frames), left.count)

        return AppleAnalysisSnapshot(
            sampleRate: metadata.sample_rate,
            sequence: metadata.sequence,
            droppedFrames: metadata.dropped_frames,
            peakLeft: metadata.peak_left,
            peakRight: metadata.peak_right,
            rmsLeft: metadata.rms_left,
            rmsRight: metadata.rms_right,
            correlation: metadata.correlation,
            dominantFrequency: metadata.dominant_frequency_hz,
            dominantLevelDB: metadata.dominant_level_db,
            spectrumDB: Array(spectrum.prefix(spectrumCount)),
            scopeLeft: Array(left.prefix(scopeCount)),
            scopeRight: Array(right.prefix(scopeCount))
        )
    }
    ...
    func startRecording(
        ...
        let result: fv1_sdk_result = url.path.withCString { path in
        ...
        guard result == FV1_SDK_OK else {
            ...
            throw AppleRecordingError.recorder(
                message.isEmpty ? "Unable to start recording." : message
            )
        }
    }

    func stopRecording() {
        fv1_apple_realtime_stop_recording(handle)
    }

    var isRecording: Bool {
        fv1_apple_realtime_is_recording(handle) != 0
    }

    func recorderStats() -> AppleRecorderStats {
        var value = fv1_apple_recorder_stats_v1()
        fv1_apple_recorder_stats_v1_init(&value)
        fv1_apple_realtime_get_recorder_stats(handle, &value)
        return AppleRecorderStats(
            rawFramesWritten: value.raw_frames_written,
            processedFramesWritten: value.processed_frames_written,
            rawFramesDropped: value.raw_frames_dropped,
            processedFramesDropped: value.processed_frames_dropped
        )
    }
    ...
    func scope(maxFrames: Int = 1024) -> ([Float], [Float]) {
        ...
        return (
            Array(left.prefix(Int(count))),
            Array(right.prefix(Int(count)))
        )
    }
}
