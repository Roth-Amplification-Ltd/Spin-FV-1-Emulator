...
Supported bootstrap hosts for Phase 6C / Linux 1.0 RC1:
...
printf 'Build type:  %s\n' "$BUILD_TYPE"
printf 'Compiler:    %s\n' "$COMPILER"
printf 'Platform:    Linux-first Phase 6C / 1.0.0-rc1 hardening\n\n'
...
if (( DO_INSTALL )); then
    if ((${#missing_commands[@]} || ${#missing_phase2[@]})); then
        if ! command -v apt-get >/dev/null 2>&1; then
            ...
            echo "Phase 6C automatic installation currently supports apt-based Linux distributions." >&2
            echo "Install a C/C++ compiler, CMake, Ninja, Python 3 (tests), Git, pkg-config, GDB, Valgrind, miniaudio headers, and SpeexDSP development files, then rerun." >&2
            exit 1
        fi
